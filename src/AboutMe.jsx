...
const REVEAL_CONTENT = [
  {
    upper: [
      ...
      "My Name is Fawwaz Aanggita Yumna Qotrunnada",
      "but you can call me Fawwaz or Vloits"
    ],
    lower: "I speak Indonesian fluently and English proficiently.",
    lower2: "Currently studying game development at vocational high school.",
  },
  {
    upper: [
      "I love gazing at the moon and stars.",
      "They're incredibly beautiful, and sometimes",
      "I wish I could reach out and touch them.",
    ],
    lower: "...though sadly, I can never quite grasp them.",
  },
  {
    upper: [
      "I sometimes drink 4–5 cups of coffee",
      "just to be able to fall asleep.",
      "For some reason, caffeine rarely affects me.",
    ],
    lower: "Yes, my body runs on reverse logic.",
  },
];
...
export default function AboutMe() {
  ...
  const { playHover, playConfirm, playCancel, playOpenDetail, playCloseDetail } = useAudio();
  ...
  const handleKey = (e) => {
    ...
    if (e.key === "Enter") { playOpenDetail(); setRevealed(true); }
    if (e.key === "ArrowRight") { playOpenDetail(); setRevealed(true); }
    ...
    <div id="menu-screen">
      ...
      {revealed && (
        <div key={`panel-${active}`} className={`sc-reveal-panel${mounted ? " mounted" : ""}`}>
          ...
          {REVEAL_CONTENT[active].lower2 && (
            <div className="sc-reveal-lower-bar" style={{ marginTop: 8 }}>{REVEAL_CONTENT[active].lower2}</div>
          )}
        </div>
      )}
      {revealed && (
